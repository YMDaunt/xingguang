...
let vm = new Vue({
    ...
    mounted: function () {
        ...
        var a = new CanvasAnimate(lr1, { length: 400, clicked: true, moveon: true })
    ...
    methods: {
        popup (type) {
            ...
            if (type == 'v') {
                ...
                this.shareLink = 'https://facebook.com/qr?id=100024293181246'
            } else if (type == 't') {
                this.shareLink = 'https://t.me/NausOfficial3'
            }
        },
        ...
        scrollEvent () {
            ...
            if (vm.getScrollTop() > 550) {
                document.querySelectorAll('.tippin-pic').forEach(function (dom) {
        ...
        submitEmail () {
            if (!/^\w+([-.]\w+)*@\w+([-]\\w+)*\.(\w+([-]\w+)*\.)*[a-z]{2,3}$/.test(this.email)) {
                layer.msg('請填寫正確的郵箱')
                return
            }
