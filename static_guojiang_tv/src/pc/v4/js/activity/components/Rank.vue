      ...
      <li v-for="(item,index) in prevList(rankList)" :class="'no'+(index+1)" :key="index">
        <div :class="{userMarginBottom:!isMod}" class="avatar-wrap">
          ...
          <div class="avatar-box">
            <img v-if="isMod" :src="item.headPic" class="avatar" @click="inlive(item.id)" onerror="this.src='//static.guojiang.tv/pc/v3/img/common/avatar/head_bg_p7_330.png'">
    ...
    <div class="myRank" v-show="showmMyRanks && isMod">
      ...
      <div class="avatar-box">
        <div class="avatar-wrap">
          <img :src="mmyRanks.headPic" alt class="avatar" @click="inlive(mmyRanks.uid)" onerror="this.src='//static.guojiang.tv/pc/v3/img/common/avatar/head_bg_p7_330.png'">
        </div>
  ...
  methods: {
    ...
    scrollLoad() {
      if (!this.entity && this.rankList.length > 0) {
        let scrollWrap = document.querySelector(".scroll-box");
        let scrollContent = document.querySelector(".last-list");
        let scrollWrapHeight = parseFloat(
          window.getComputedStyle(scrollWrap, null).getPropertyValue("height")
        );
        let that = this;
        this.entity = new PolyfillScroll({
          scrollWrap: ".scroll-box",
          scrollContent: ".last-list",
          bar: {
            width: "10px",
            height: "50px",
            right: "17px",
          ...
          cb(scrollTop) {
            const contentHeight = parseFloat(
              window
                .getComputedStyle(scrollContent, null)
                .getPropertyValue("height")
            );
            if (contentHeight - scrollTop - scrollWrapHeight < 500) {
