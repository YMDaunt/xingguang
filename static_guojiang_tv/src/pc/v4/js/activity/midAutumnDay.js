...
var initPCScroll = function (ele, right) {
    $(ele).niceScroll({ // eslint-disable-line
        cursorwidth: 8,
        cursorcolor: 'rgba(255, 255, 255, .9)', // 设置滚动条滑块的颜色
        cursorborder: 'none', // CSS方式定义滚动条边框颜色
        autohidemode: false,
        cursorfixedheight: 40,
        horizrailenabled: false,
        hwacceleration: true,
        railpadding: { top: 0, right: right || 20, left: 0, bottom: 4 }
    })
}
...
new Vue({
    ...
    methods: {
        ...
        idGiveInput (evt) {
            ...
            service.getUserNameByIdBounce(num, (data) => {
                if (data.errno === 0) {
                    ...
                    // this.userInput.giveInputTips = data.data.msg || '*请确认用户ID是否正确'
        ...
        idAskInput (evt) {
            ...
            service.getUserNameByIdBounce(num, (data) => {
                if (data.errno === 0) {
                    ...
                    // this.userInput.askInputTips = data.data.msg || '*请确认用户ID是否正确'
        ...
        initList () { // 初始化列表加载
            var scrollBox = function (ele, cb, ctx, scrollRight) {
                var bh = 150
                var _self = ctx
                var scrollEle = ele
            ...
            // 3. 字块中心 -> 收到字块
            scrollBox(this.$refs.takelist, this.getCardsTakeList, this, 10)

            // 4. 字块中心 -> 索要信息
            scrollBox(this.$refs.asklist, this.getCardsAskList, this, 4)

            // 5. 字块中心 -> 赠送记录
            scrollBox(this.$refs.givelist, this.getCardsPresentList, this, 10)

            // 6. 其余列表的滚动条样式修改
            initPCScroll(this.$refs.rulelist, 16)
        },
