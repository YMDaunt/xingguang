...
function MyPCScroller (node) {
    ...
    // 默认阻止冒泡
    // this.bubble = typeof bubble === 'undefined' ? true : bubble
    ...
    this._isFF = typeof this.node.onmousewheel === 'undefined'
    this._addMWEvent(this.node, this._scroll)
    this._addDragEvent(this.ssThumb)

    con.scroller.push(this)
}

MyPCScroller.prototype = {
    // 样式重置
    _reset () {
    ...
    _addDragEvent (element, start, end, draging) {
        element.addEventListener('mousedown', (evt) => {
            this.draging = true
            this.dragSY = evt.pageY
            console.log('start', evt.pageY)
        })
        ...
        var dragScroller = (evt) => {
            if (!this.draging) return

            var deltaY = evt.pageY - this.dragSY // 滚动差值
            this.dragSY = evt.pageY
            ...
            var maxH = this.ss.clientHeight - this.boxSize.ssHeight
        ...
        element.addEventListener('mousemove', dragScroller)
        document.body.addEventListener('mousemove', dragScroller)
    },
    refreshDOM () {
        this.draging = false
...
var pcMixins = {
    ...
    methods: {
        renderScroller () {
            new MyPCScroller(document.querySelector('.layer-wrap'))

            var ltContent = document.querySelectorAll('.lt-content')
            new MyPCScroller(ltContent[0])
            new MyPCScroller(ltContent[1])
            new MyPCScroller(ltContent[2])

            var tabM = document.querySelectorAll('.tab-h-m')
            new MyPCScroller(tabM[0])
            new MyPCScroller(tabM[1])
            new MyPCScroller(tabM[2])

            var tabC = document.querySelectorAll('.list-table-content')
            // new MyPCScroller(ruleTab)
            new MyPCScroller(tabC[1])
            new MyPCScroller(tabC[2])
        },
        updateScroller () {
            con.scroller.forEach(item => {
                item.refreshDOM()
...
var $v = new Vue({
    ...
    methods: {
        ...
        initScrollBox () {
            ...
            scrollBox(this.$refs.modlist.$refs.list, this.loadModRanks, this)
            scrollBox(this.$refs.userlist.$refs.list, this.loadUserRanks, this)
            // 夺宝记录懒加载
