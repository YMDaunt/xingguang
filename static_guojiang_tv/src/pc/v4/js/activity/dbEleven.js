...
if (os.isPhone) location.href = '//m.kuaishouvideo.com/dist/activity/dbEleven.html'
var ruleJudge = true
...
new Vue({
    ...
    methods: {
        ...
        ruleShow () {
            ...
            Vue.nextTick(() => {
                if (ruleJudge) {
                    const sortWrapElement = document.querySelector('.rule-box-out')
                    const sortWrapHeight = parseFloat(window.getComputedStyle(sortWrapElement, null).getPropertyValue('height'))
                    const sortListElement = document.querySelector('.rule-box-con')
                    that.transformEntity1 = new PolyfillScroll({
                        scrollWrap: '.rule-box-out',
                        scrollContent: '.rule-box-con',
                        bar: {
                            width: '8px',
                            height: '60px',
                            right: '6px',
        ...
        doPkSvg () {
            this.svgPlayer = !this.svgPlayer
            playSvg('//static.guojiang.tv/src/pc/svg/activity/dbEleven/qrl/data.json')
        },
        doLbSvg () {
            this.svgPlayer = !this.svgPlayer
            playSvg('//static.guojiang.tv/src/pc/svg/activity/dbEleven/qrls/data.json')
        },
...
$('.top-other').niceScroll({
    cursorwidth: 8,
    cursorcolor: 'rgba(214, 57, 51,0.8)', // 设置滚动条滑块的颜色
