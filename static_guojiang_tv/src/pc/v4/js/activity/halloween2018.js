...
var pcMixins = {
    methods: {
        ...
        renderScroller () {
            [
                this.$refs.mDay1List.$refs.list,
                this.$refs.mDay2List.$refs.list,
                this.$refs.mDay3List.$refs.list,
                this.$refs.mList.$refs.list,
                this.$refs.userList.$refs.list
            ].forEach((item) => {
                $(item).niceScroll({
                    cursorwidth: 8,
                    cursorcolor: '#c1b6fb', // 设置滚动条滑块的颜色
            ...
            con.scrollers[0] = $('.modal-rules .xg-modal-bg-c').niceScroll({
                cursorwidth: 8,
                cursorcolor: '#c1b6fb', // 设置滚动条滑块的颜色
                cursorborder: 'none', // CSS方式定义滚动条边框颜色
                autohidemode: false,
                ...
                horizrailenabled: false,
                hwacceleration: true,
                railpadding: { top: 40, right: 6, left: 0, bottom: 0 },
                zindex: 23
            })

            con.scrollers[1] = $('.modal-rewards .xg-modal-bg-c').niceScroll({
                cursorwidth: 8,
                cursorcolor: '#c1b6fb', // 设置滚动条滑块的颜色
                cursorborder: 'none', // CSS方式定义滚动条边框颜色
                autohidemode: false,
...
new Vue({
    ...
    methods: {
        ...
        showModal (type) {
            ...
            if (type === 0) {
                // console.log(con.scrollers[0])
                // window.scrollers = con.scrollers // for debug
                setTimeout(() => {
                    con.scrollers[1].hide()
                    con.scrollers[0].show().doScrollTo(con.scrollers[0].scrollTop())
                })
            } else if (type === 1) {
                setTimeout(() => {
                    con.scrollers[0].hide()
                    con.scrollers[1].show().doScrollTo(con.scrollers[1].scrollTop())
                })
            } else {
                setTimeout(() => {
                    con.scrollers[0].hide()
