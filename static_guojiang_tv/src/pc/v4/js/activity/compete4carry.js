...
var service = {
    ...
    getFinalHoursList (page, size) { // 决赛小时榜
        return axios.get('/centerMatch/getRanks', {
            params: {
                stage: 3,
                type: 'hour'
...
/* util */
var assign = function (target, source) { // 对象赋值
...
var formatDate = function (value, sep, hasHours) { // 计算剩余时间
...
new Vue({
    ...
    mounted: function () {
        ...
        this.renderAllScroller()
    },
    methods: {
        /* 美化滚动条 */
        renderAllScroller () {
            [ '.modal-rewards .xg-box-c', this.$refs.rulesList ].forEach((item, index) => {
                con.scroller[index] = $(item).niceScroll({
                    cursorwidth: 8,
                    cursorcolor: 'rgba(255, 46, 138, .8)', // 设置滚动条滑块的颜色
                    cursorborder: 'none', // CSS方式定义滚动条边框颜色
                    autohidemode: false,
                    cursorfixedheight: 40,
                    horizrailenabled: false,
                    hwacceleration: true,
                    railpadding: { top: 6, right: 24, left: 0, bottom: 0 },
                    zindex: 32
                })
            });
            [ '.final-history', '.promote-history', '.tb-pre-days', '.tb-users', '.tb-hours', '.tab-c-days .box-final', '.tab-c-zhubos .box-final' ].forEach(item => {
                $(item).niceScroll({
                    cursorwidth: 8,
                    cursorcolor: 'rgba(255, 46, 138, .8)', // 设置滚动条滑块的颜色
                    cursorborder: 'none', // CSS方式定义滚动条边框颜色
                    ...
                    horizrailenabled: false,
                    hwacceleration: true,
                    railpadding: { top: 6, right: 2, left: 0, bottom: 0 },
                    zindex: 19
                })
            })
        },
