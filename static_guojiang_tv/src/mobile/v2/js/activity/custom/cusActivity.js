...
new Vue({
        ...
        longtap: {
            bind: function (el, binding) {
                ...
                el.addEventListener('touchstart', function () {
                    tapedTimer = setTimeout(function () {
                        clearTimeout(tapedTimer)
                        tapedTimer = null
                        binding.value(el.getAttribute('data-attr'))
                    }, 500)
                }, false)
                ...
                function click (evt) {
                    if (!tapedTimer) {
                        evt.stopPropagation() // 如果是长按 则不触发click
                    }
                    clearTimeout(tapedTimer)
                }

                el.addEventListener('touchmove', cancelTap, false)
                el.addEventListener('touchend', click, false)
                el.addEventListener('touchcancel', cancelTap, false)
            }
        }
    },
    filters: {
    ...
    mounted: function () {
        ...
        var wH = window.innerHeight || 580
        var wW = window.innerWidth || 360
        if (wH / wW < 1.65) {
            console.log('监听滚动')
            var _self = this
    ...
    methods: {
        ...
        ignore (evt) { // 全局取消宝箱说明框
            if (evt.target.className.indexOf('tb-box-toast') !== -1) {
                return
            }
            this.ui.bxInfoShowInd = '0'
        },
        needsClick (evt) { // input click
            evt.target.focus()
        },
        ...
        runMsg () { // 消息滚动条
            this.$nextTick(() => {
                ...
                // 不支持css3 animation (webkitAnimation 尽管有值，在小部分旧机型上动画显示很卡顿)
                // if (document.getElementsByTagName('body')[0].style.animation === undefined) {
                var msgWrapper = document.getElementsByClassName('run-msg-wrapper')[0]
                var left = 0
                var step = 0.4
                ...
                //     // this.app.msgStyle['animation'] = 'msgScroll ' + speed + 's linear 0s infinite normal none running'
                //     // this.app.msgStyle['-webkit-animation'] = 'msgScroll ' + speed + 's linear 0s infinite normal none running'
                //     this.$refs.msgwrap.style.animation = 'msgScroll ' + speed + 's linear 0s infinite normal none running'
                //     this.$refs.msgwrap.style.webkitAnimation = 'msgScroll ' + speed + 's linear 0s infinite normal none running'
                // }
            })
        },
        initList () { // 加载礼物列表
