// 在loader里面完成less的编译
import '../../../css/activity/channel/jiujiu2.less'
...
window.vm = new Vue({
    el: '#app',
    data: {
        activityEndTime: '2017-10-20 23:59:59',
        packageId: 2,
        activeDisplay: '', // 是否显示下载框
        channel: 'and-yybllhd-7', // 渠道号测试环境developer-default
        tel: '', // 手机号
        authCode: '', // 验证码
        nickname: '', // 昵称
        password: '', // 密码
    ...
    created: function () {
        ...
        this.setDownloadBar()
        // 测试
        // this.registerSucessCallback();
    },
    methods: {
        getActivityState () {
            let that = this

            axios.get('/channel/getServerTime')
                .then((res) => {
				    let data = res.data

                    if (data.errno == 0) {
                        that.endTimeCallback(data.data.nowTime)
                    }
			    })
        },
        endTimeCallback (serverTime) {
            let diffTime = common.exchangeTime(serverTime) - common.exchangeTime(this.activityEndTime)
            if (diffTime > 0) {
                layer.open({
                    content: '抱歉，活动已结束。'
                })
