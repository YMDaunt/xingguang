...
if (os.isPc) location.href = '//www.kuaishouvideo.com/activity/pkSong.html'
let linkJudge = false
var socket = null
var eightSwiper = null
...
new Vue({
        ...
        axios.get('/Solstice/initInfo')
            .then(res => {
                ...
                socket.onopen = function () {
                    socket.send(JSON.stringify({'cmd': 'querySolsticeRank', data: {}}))
                    socket.send(JSON.stringify({'cmd': 'querySolsticeRank',
                        data: {
                            sortBy: 'isPlaying'
                        }}))
                ...
                if (that.isPk) {
                    axios.get('/Solstice/pkInfo?rid=' + that.roomId)
                        .then(res => {
                            ...
                            that.giftArr = data.data.currentScore
                        })
                        .catch(err => {
                            console.log(err)
                        })
    ...
    methods: {
        tz () {
            var type = common.getPlatformType()
            if (type == 'ios_webview') {
                        ...
                        (res) => {
                            ...
                            if (data.errno == 0) {
                                if (data.data.useApplePay) {
                                    ...
                                    window.location.href = '/rechargeApp'
                                }
                            }
                        },
                        (err) => {
        ...
        change (e, index) {
            ...
            if (url == that.videoUrl) {
                ...
                socket = new WebSocket(that.wsUrl)
                socket.onopen = function () {
                    socket.send(JSON.stringify({'cmd': 'querySolsticeRank', data: {}}))
                    socket.send(JSON.stringify({'cmd': 'querySolsticeRank',
                        data: {
                        ...
                        Vue.nextTick(() => {
                            for (var i = 0; i < that.videoArr.length; i++) {
                                if (rid == that.videoArr[i].rid) {
                                    that.playingNum = i
                                    that.roomId = that.videoArr[i].rid
